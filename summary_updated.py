...
# Function to preprocess the text blocks
def preprocess_block(block):
    lemmatizer = WordNetLemmatizer()
    stop_words = set(word.lower() for word in stopwords.words('english')).union(set(ENGLISH_STOP_WORDS))
    ...
    for sentence in block:
        ...
        # Lemmatize and filter out stop words, numbers, and non-alphanumeric tokens
        words = [lemmatizer.lemmatize(word) for word in words if word.isalnum() and word.lower() not in stop_words and not word.isdigit()]
        
        # Perform POS tagging and NER
        tagged_words = pos_tag(words)
        named_entities = ne_chunk(tagged_words, binary=True)
        
        # Filter out verbs (VB, VBD, etc.) and adverbs (RB, RBR, RBS)
        filtered_words = []
        for word, pos in tagged_words:
            if pos not in ('VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ', 'RB', 'RBR', 'RBS') and word.lower() not in stop_words:
                filtered_words.append(word)
        
        # Extract and combine named entities into single units
        filtered_entities = []
        combined_entity_words = set()
        for chunk in named_entities:
            if hasattr(chunk, 'label') and chunk.label() == 'NE':
                entity = " ".join([word for word, pos in chunk.leaves()])
                filtered_entities.append(entity)
                combined_entity_words.update([word for word, pos in chunk.leaves()])
        
        # Ensure no entities or important words are mistakenly filtered out
        final_words = []
        for word in filtered_words:
            if word not in combined_entity_words:
                final_words.append(word)
        
        # Combine final words with entities
        final_words.extend(filtered_entities)
        
        # Update word frequencies
        for word in final_words:
            word_frequencies[word] += 1

        # Store the final words for this sentence
        sentence_to_words.append((sentence, final_words))
...
def preprocess_all_blocks(blocks):
    ...
    for block_name, block in blocks.items():
        word_frequencies, sentence_to_words = preprocess_block(sent_tokenize(block))
